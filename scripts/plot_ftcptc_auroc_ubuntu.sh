...
PYTHON_BIN="python3"
OUTPUT_PATH="output/figures/FTCPTC_auroc.pdf"
DOCTOR_JSON=""
TITLE=""
...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/plot_ftcptc_auroc_ubuntu.sh \
    --inputs ftc_model_a.json ftc_model_b.json ftc_model_c.json \
    --labels ThyroidAgent BestSingleModel AblationModel \
    [--doctor-json ./doctor_multi_task_metrics.json] \
    [--output output/figures/FTCPTC_auroc.pdf] \
    [--python python3] \
    [--title "FTCPTC AUROC Comparison"]

Required:
  --inputs       FTCPTC 任务的一个或多个 results JSON
  --labels       与输入 JSON 一一对应的标签

Optional:
  --doctor-json  医生读片点汇总 JSON；传入后会叠加 FTCPTC 医生点
  --output       输出路径，默认 output/figures/FTCPTC_auroc.pdf
...
Example based on my_run.md:
  bash scripts/plot_ftcptc_auroc_ubuntu.sh \
    --inputs \
      ...
      /mnt/wangbd8/workspace/ThyroidAgent/Classification_Agent/Malignant_Cls_Agent/output/test_dataset/FTCPTC/no_llm_SingleModel/results_20260523_234359.json \
...
if ((${#INPUTS[@]} == 0)); then
  echo "FTCPTC 没有提供任何 JSON 输入。" >&2
  exit 1
fi
if ((${#LABELS[@]} == 0)); then
  echo "FTCPTC 没有提供任何标签。" >&2
  exit 1
fi
if ((${#INPUTS[@]} != ${#LABELS[@]})); then
  echo "FTCPTC 的 JSON 数量与标签数量不一致。" >&2
  exit 1
fi
