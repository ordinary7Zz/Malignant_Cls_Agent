...
def find_corresponding_mask(
    ...
    """在掩码目录中查找与图像对应的掩码（同名，后缀可不同）。"""
    image_extensions = ['.jpg', '.jpeg', '.png', '.bmp', '.tif', '.tiff']
    all_exts = image_extensions + [ext.upper() for ext in image_extensions]

    candidates: list[Path] = []

    # 优先按相对路径匹配（mask_dir / 相对filename），适配 label_file 驱动模式。
    if image_root is not None:
        try:
            ...
            rel = None
        if rel is not None:
            candidates.append(mask_dir / rel)
            for ext in all_exts:
                candidates.append(mask_dir / rel.parent / f"{rel.stem}{ext}")

    # 兼容旧逻辑：按 basename 在 mask 根目录下匹配。
    candidates.append(mask_dir / image_path.name)
    for ext in all_exts:
        candidates.append(mask_dir / f"{image_path.stem}{ext}")
...
def resolve_label_key(paths_config: dict) -> Optional[str]:
    """解析可选标签键名 data.label_key。"""
    data_cfg = paths_config.get("data", {}) if isinstance(paths_config, dict) else {}
    raw = data_cfg.get("label_key", None)
    if raw is None:
        return None
    s = str(raw).strip()
...
def load_valid_filenames_from_label_file(label_path: Path, label_key: str) -> List[str]:
    """
    从 label_file 中读取当前任务有效样本文件名（标签值 != -1）。
    支持：
    - list[dict]：每项包含 filename/image_name 与 label_key
    - dict：{filename: int} 或 {filename: {label_key: int}}
    """
    ...
    if isinstance(data, list):
        for item in data:
            ...
            if fn is None or raw_label is None:
                continue
            try:
...
def main(config_path: str = "config/config.yaml"):
    ...
    configured_label_key = resolve_label_key(paths_config)
    use_label_driven_scan = resolved_label_path is not None and configured_label_key is not None
    ...
    if use_label_driven_scan:
        if not image_input_path.is_dir():
            print("✗ 当同时提供 label_file 与 label_key 时，data.image_input 必须是图像根目录")
            return

        valid_filenames = load_valid_filenames_from_label_file(resolved_label_path, configured_label_key)
        print(f"✓ 检测到 label_file + label_key，按标签筛选到 {len(valid_filenames)} 个有效样本")

        image_files: List[Path] = []
        missing_files: List[str] = []
        for fn in valid_filenames:
            rel_path = Path(str(fn).replace('\\', '/'))
            candidate = (image_input_path / rel_path).resolve()
            if candidate.exists() and candidate.is_file():
                image_files.append(candidate)
            else:
                missing_files.append(str(rel_path))

        image_root_dir = image_input_path.resolve()
        print(f"✓ 基于标签文件定位到 {len(image_files)} 个可用图像文件")
        if missing_files:
            print(f"⚠️ 有 {len(missing_files)} 个标签样本未在 image_input 下找到对应图像")
    else:
        # 回退：未提供 label_file+label_key 时，按 image_input 全量扫描。
        if image_input_path.is_file():
            image_files = [image_input_path.resolve()]
            image_root_dir = image_input_path.parent.resolve()
            print(f"✓ 检测到单个图像文件")
    ...
    for idx, image_file in enumerate(image_files, 1):
        ...
        try:
            ...
            mask = None
            if mask_dir:
                mask_file = find_corresponding_mask(
                    image_file,
                    mask_dir,
    ...
    if len(all_results) > 0:
        ...
        def _load_labels(label_path: Path, label_key: Optional[str] = None) -> dict[str, int]:
            with open(label_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            ...
            if isinstance(data, list):
                for item in data:
                    if not isinstance(item, dict):
                        continue
                    fn = item.get("filename") or item.get("image_name")
                    raw_label = item.get(label_key) if label_key else item.get("malignancy", item.get("label"))
                    if fn is None or raw_label is None:
                        continue
                    try:
                        label_int = int(raw_label)
        ...
        try:
            ...
            if label_path is None or not label_path.exists():
                ...
                if configured_label_key:
                    print(f"\n【平均分类指标】标签键: {configured_label_key}（-1 将跳过）")
                else:
                    print("\n【平均分类指标】标签键: malignancy/label（兼容模式，-1 将跳过）")
